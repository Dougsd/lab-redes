...
Menu(){
	...
	echo "[ 4 ] CONFIGURAR O DNS"
	echo "[ 5 ] COLOCAR HOSTNAME NO /ETC/HOSTS"
	echo -e 'DHCP##########' 
	echo "[ 6 ] CRIA NOVO SCOPO E RESERVA (APAGA O SCOPO EXISTENTE CASO HAJA)"
	echo "[ 7 ] ADICIONAR RESERVA A SCOPO JA EXISTENTE"
	echo -e 'DNS##########' 
	echo "[ 8 ] CRIA NOVA ZONA(APAGA AS ZONAS EXISTENTES CASO HAJA)"
	echo "[ 9 ] ADICIONAR HOST A UM ZONA JA EXISTENTE"
	echo "[ 0 ] SAIR"
...
Novo(){
	...
	#Adaptado a rede ipv4 113.167.9 e ipv6 cafe:: do Lab de Redes
	...
	echo "Diga o NOME da maquina a ser adicionada: "
	read nome
	echo "Diga o IPv4 da maquina: "
	read ip4
	echo "Diga o IPv6 da maquina: "
	read ip6
	echo "Diga um apelido a ela: "
	read apelido
...
Dns(){
	echo "Diga o DNS desejado (Para que o servidor DNS funcione, seu ip tem que ser o DNS, seu ip atual é $_ip4) "
	read _dns
	_dnsP=$(echo $_dns)
	echo -e "# Generated by NetworkManager\nnameserver $_dnsP\nnameserver 113.167.9.17" > /etc/resolv.conf
}
